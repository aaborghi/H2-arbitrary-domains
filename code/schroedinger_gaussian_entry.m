...
dynamics = @(t,x,A,B) A*x+B*exp(-(t-2)^2/0.1)-2*B*exp(-(t-5)^2/0.01);
...
[~,xr_irka] = ode23(dynamics,linspace(0,5,1000),zeros(r,1),options,Ar_irka,Br_irka);
y2 = Cr*xr.';
y3 = Cr_irka*xr_irka.';
error = y1-y2;
error_irka = y1-y3;
inputu = @(t) exp(-(t-2).^2./0.1)-2*exp(-(t-5).^2./0.01);
